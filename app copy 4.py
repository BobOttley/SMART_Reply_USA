...
def delete_pdf():
    ...
    try:
        # Step 1: Delete the actual PDF file
        ...
        # Step 2: Remove associated chunks from metadata
        ...
        for i, chunk in enumerate(metadata):
            ...
            if not source.endswith(fname):
                new_metadata.append(chunk)
                new_embeddings.append(doc_embeddings[i])

        print(f"🧼 Original chunks: {len(doc_embeddings)} → After delete: {len(new_embeddings)}")

        # Step 3: Update in-memory structures
        metadata = new_metadata
        doc_embeddings = np.array(new_embeddings)

        # Step 4: Save updated metadata.pkl
        with open("embeddings/metadata.pkl", "wb") as f:
            pickle.dump({
                "messages": metadata,
        ...
        return jsonify({"status": "✅ PDF deleted and removed from knowledge base"})
