...
def rename_pdf():
    try:
        ...
        # Secure the new filename and ensure it ends with .pdf
        ...
        try:
            ...
            # Update metadata entries with the old URL
            for item in metadata:
                if not isinstance(item, dict):
                    print(f"⚠️ Skipping invalid metadata entry during rename: {item}")
                    continue
                if item.get("url") == f"/uploaded_pdfs/{old_filename}":
                    item["url"] = f"/uploaded_pdfs/{new_filename}"
                    item["name"] = new_filename
                    item["source"] = f"uploaded_pdfs/{new_filename}"
            ...
            # Continue even if metadata update fails, as file is renamed
...
def delete_pdf():
    try:
        ...
        for m, e in zip(metadata, doc_embeddings):
            # Ensure m is a dictionary; skip if not
            if not isinstance(m, dict):
                print(f"⚠️ Skipping invalid metadata entry: {m}")
                continue
            source = m.get("source", "")
            # Normalize source to match filename
            source_filename = os.path.basename(source) if source else ""
            if fname != source_filename:
                new_metadata.append(m)
                new_embeddings.append(e)
            else:
                print(f"🗑️ Removing metadata entry for {source}")
        ...
        for m in new_metadata:
            if not isinstance(m, dict):
                print(f"⚠️ Invalid metadata entry after filtering: {m}")
                continue
            source = m.get("source", "")
            source_filename = os.path.basename(source) if source else ""
            if fname == source_filename:
                print(f"⚠️ Still contains reference to deleted file: {source}")

        # Update global state
        metadata.clear()
        metadata.extend(new_metadata)
        # Reassign doc_embeddings to new array
        ...
        return jsonify({"message": f"🗑️ Deleted file: {fname}"}), 200
